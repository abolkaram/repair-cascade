...
def f(x):
 ...
 if isinstance(x,list):
  for y in x:
   z=f(y)
   if z:return z
a=create_account(account_private_key=v('ACCOUNT_5_GENLAYER_PRIVATE_KEY'));c=create_client(chain=studionet,account=a);h=c.deploy_contract(code=(R/'contracts/contract.py').read_text(encoding='utf-8'),args=[]);r=c.wait_for_transaction_receipt(transaction_hash=h,status='FINALIZED',retries=180,interval=5000);print(json.dumps({'contract':f(r),'tx':h,'wallet':a.address,'status':r.get('status_name')}),flush=True)
